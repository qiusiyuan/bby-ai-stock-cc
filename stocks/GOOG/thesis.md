...
## Industry & TAM
Search advertising is ~$300B/yr globally and growing ~8–10% annually. Google captures ~90%+ of that. YouTube advertising is ~$35–40B/yr and growing ~10–15% (now larger than Netflix by revenue). Google Cloud is competing in an IaaS/PaaS market projected to reach $1T+ by 2030 — currently at ~$42B run rate. Waymo's autonomous rideshare TAM is early but potentially transformational ($1T+ long-run). The AI TAM is genuinely unbounded — Alphabet's position across model training, inference, and enterprise deployment makes it a core beneficiary.
...
- DOJ wins structural remedy requiring sale of Chrome, Android, or forced separation of Search and advertising
- Google Cloud loses market share to AWS/Azure for two consecutive quarters (indicates competitive failure in AI infra)
- Gemini lags ChatGPT/Claude in model quality benchmarks by more than one generation for 12+ months
- Capital allocation shifts to a large, value-destructive acquisition (over $50B in a tangential space)

## Position
- Target: $430 (base case)
- Thesis-break level: $280 (~24% below current; forces re-read)
...
### 2026-07-31  Note — +5.99% 是板块 re-rating 溢出，不是自身改善（0.76x 缩量）

**事实：** GOOG $353.68 (+5.99%, **0.76x 缩量**)，5d +10.84%，30d -2.23%（财报当日曾一度 -10%）。

**修复机制：不是 GOOG 有新消息，是基准变了。** 逻辑链：AMZN 当天证明 AI capex → 云收入加速的传导真实（AWS +37%，18季最快）→ 市场对"云 + AI"组合的整体信心上修 → 在新基准下 GOOG Cloud +28% 不再是"证明变现失败"，而是"变现进行中，只是不如 AWS/Azure 陡" → 上周杀过头的部分被修复。

**0.76x 成交量是关键限定：缩量上涨 5.99% = 空头回补 + 错杀修复，不是机构新建仓。** 这意味着这波修复的天花板就是"回到财报前水平"（约 $360-365），不会更高 —— **因为真正的多头论点（Search AI 变现可见性）今天没有任何改善。GOOG 需要的是自己的证据，不是 AMZN 的。**

**7月的规律在 GOOG 身上完整验证了一遍（正反两次）：** 市场只为"当期可计量的收入加速"付钱。财报当日 GOOG 因 AI 嵌入 Search 不透明被罚 -10%（MSFT/AMZN 因当期计费收入被奖）；今天基准上移后又被修复。**这不是投资者偏好善变，是同一条规则在不同基准下的两次应用。** 7/13 记录的那个观察点（管理层敢不敢量化 AI revenue）依然是 GOOG 的核心变量 —— 至今未解决。

**雷达项（长期，非今日驱动）：AI 搜索对传统搜索的侵蚀。** 当天 Reddit 起诉 Perplexity / SerpApi 的反规避条款诉讼获法院推进，文中提及 "Google search traffic fluctuations"。这是结构性主线，不是今天的驱动因素，但要持续跟踪。

**立场：持有。** 大盘股，估值修复到财报前即到位。真正的 re-rating 需要 GOOG 自己首次量化 AI 收入 —— 那是下一个催化，时间未定。
